...
while True:
    ...
    decisao = str(input('voce quer entrar ou se registrar?[E/R/X] ')).upper().strip()[0]   
    ...
    with open('superdb.json', 'w', encoding='utf-8') as db: #msm coisa que o outro mas pra escrever
        json.dump(dados, db, indent=4, ensure_ascii=False)   #o indent=4 serve pra ele usar a identacao padrao e pra ficar alinhado, o ensure_ascii=True serve pra aceitar melhor o alfabeto portugues-